...
namespace SDK
{
	...
	static_assert(sizeof(FOverriddenPropertySet) == 0x000058, "Wrong size on FOverriddenPropertySet");
	...
	public:
		struct FQuat Quat()
		{
			...
			constexpr float HALF = 0.5f * DEG_TO_RAD;

			const float NewPitch = this->Pitch * HALF;
			const float NewYaw = this->Yaw * HALF;
			const float NewRoll = this->Roll * HALF;

			const float SP = std::sin(NewPitch);
			const float CP = std::cos(NewPitch);

			const float SY = std::sin(NewYaw);
			const float CY = std::cos(NewYaw);

			const float SR = std::sin(NewRoll);
			const float CR = std::cos(NewRoll);
