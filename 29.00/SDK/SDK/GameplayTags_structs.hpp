...
public:
	...
	bool HasAll(const FGameplayTagContainer& ContainerToCheck) const
	{
		...
		{
			auto& Tag = ContainerToCheck.GameplayTags.Get(i);

			bool Found = false;
			for (int x = 0; x < GameplayTags.Num(); x++)
			{
				auto& Tag2 = GameplayTags.Get(x);
	...
	void AppendTags(const FGameplayTagContainer& Other)
	{
		...
		{
			auto& GameplayTag = Other.GameplayTags.Get(i);
		...
		{
			auto& ParentTag = Other.ParentTags.Get(i);
